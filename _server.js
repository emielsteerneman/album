...
if(false){
    ...
    // let filename = "000442a8e4f1f5b8d3f147e80840ac02eb1679bd$8_8_AAVUiIiHiIZmMzIiAAVVmYiIiIh3h3MyAAVViIiIiIiIiHMyEQVVmYqqmZqpiIMyAAREiImZmZqqmYMyAAIhd3mYiIiHd2IiAAAAEQRDZld3d2IiAAAAAAEQEQEQEQAA$IMG_20160509_232917.jpg";
    // let filename_original = "IMG_20160509_232917.jpg";
    // let date = toolbox.getDateFromFilepath({filename})
    // l(date);
    // let hashes = _.map(files, file => file.filename.split("$")[0]);
    // let uniques = _.uniq(hashes);
    // l(`Hashes : ${hashes.length}, uniques : ${uniques.length}, duplicates : ${hashes.length - uniques.length}`);
...
if(true ){
    ...
    let album = new Album({
        dir : "/home/emiel/Desktop/hugeasscalendar2/"
        , config
    });

    let files = toolbox.getFilesInDir({dir : "/home/emiel/Desktop/hugeassalbum"});

    l(`Files found : ${files.length}`);
    ...
    (function runBatch(){
        ...
        // Resolve promises
        Promise.all(_.map(promisesBatch, prom => prom.reflect())).then(() => {
            l(`\tPromises from batch [${batchAt}, ${batchAt + batchSize}] resolved`);
            totalSize += _.sum(_.map(filesBatch, file => file.filestat.size));
            batchAt += batchSize;
            if(batchAt < files.length){
                setTimeout(runBatch, 100);
